...
class FindEntityRequest(BaseToolRequest):
    ...
    name: str = Field(
        ...,
        description="The name of the entity to search for (e.g., 'PaymentService').",
    )
    entity_type: Optional[str] = Field(
        None, description="Optional type of the entity (e.g., 'class', 'function')."
    )
...
class GetRelationshipsRequest(BaseToolRequest):
    ...
    relationship_type: Optional[str] = Field(
        None, description="Filter by edge type (e.g., 'CALLS', 'IMPLEMENTS')."
    )
    direction: DirectionEnum = Field(
        default=DirectionEnum.BOTH, description="Traversal direction."
    )
...
class FindDependenciesRequest(BaseToolRequest):
    ...
    entity_name: str = Field(
        ..., description="The entity name to find dependencies for."
    )
...
class HybridSearchRequest(BaseToolRequest):
    ...
    top_k: int = Field(
        default=5, ge=1, le=20, description="Number of context chunks to return."
    )
    filters: Optional[Dict[str, Any]] = Field(
        default=None, description="Metadata filters (e.g., repository or file type)."
    )
...
class GroundedAnswerResponse(BaseModel):
    ...
    flow: List[str] = Field(
        ...
        description="Ordered steps representing system flow or architecture.",
    )
    evidence: List[str] = Field(
        ..., description="List of specific sources and Knowledge Graph facts used."
    )
